 ...
 * rights. The notifier should display a message and a button "login" that calls usePopupLogin().login()
 *
 * @param state the current login state (to distinguish if user is not logged in or logged in with no rights)
...
export interface UsePopupLoginOptions<UserAuthenticationState extends AuthenticationState> {
     ...
     *
     * The notifier returns either a Promise resolving to true, meaning that user clicked on the login button,
